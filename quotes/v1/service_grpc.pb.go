...
func (c *quoteServiceClient) GetQuotesForAsset(ctx context.Context, in *GetQuotesForAssetRequest, opts ...grpc.CallOption) (QuoteService_GetQuotesForAssetClient, error) {
	stream, err := c.cc.NewStream(ctx, &QuoteService_ServiceDesc.Streams[0], "/protos.frontend.quotes.v1.QuoteService/GetQuotesForAsset", opts...)
	if err != nil {
		return nil, err
	}
...
func (c *quoteServiceClient) GetPriceLadderForAsset(ctx context.Context, in *GetPriceLadderForAssetRequest, opts ...grpc.CallOption) (QuoteService_GetPriceLadderForAssetClient, error) {
	stream, err := c.cc.NewStream(ctx, &QuoteService_ServiceDesc.Streams[1], "/protos.frontend.quotes.v1.QuoteService/GetPriceLadderForAsset", opts...)
	if err != nil {
		return nil, err
	}
	x := &quoteServiceGetPriceLadderForAssetClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type quoteServiceGetPriceLadderForAssetClient struct {
	grpc.ClientStream
}

func (x *quoteServiceGetPriceLadderForAssetClient) Recv() (*data.PriceLadderEntry, error) {
	m := new(data.PriceLadderEntry)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
func (UnimplementedQuoteServiceServer) GetPriceLadderForAsset(*GetPriceLadderForAssetRequest, QuoteService_GetPriceLadderForAssetServer) error {
	return status.Errorf(codes.Unimplemented, "method GetPriceLadderForAsset not implemented")
}
...
func _QuoteService_GetPriceLadderForAsset_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetPriceLadderForAssetRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(QuoteServiceServer).GetPriceLadderForAsset(m, &quoteServiceGetPriceLadderForAssetServer{stream})
}
...
type quoteServiceGetPriceLadderForAssetServer struct {
	grpc.ServerStream
}

func (x *quoteServiceGetPriceLadderForAssetServer) Send(m *data.PriceLadderEntry) error {
	return x.ServerStream.SendMsg(m)
}
